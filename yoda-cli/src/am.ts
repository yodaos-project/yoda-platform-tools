...
program
  ...
  .action(async (url, cmd) => {
    const result = await Command('OpenUrl', [url], DBusConnection, cmd.serial)
    printResult(result)
  })
...
program
  ...
  .action(async (packageName, cmd) => {
    const result = await Command('LaunchApp', [packageName], DBusConnection, cmd.serial)
    printResult(result)
  })
...
program
  ...
  .action(async (packageName, cmd) => {
    const result = await Command('ForceStop', [packageName], DBusConnection, cmd.serial)
    printResult(result)
  })
